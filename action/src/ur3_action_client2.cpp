...
bool moveUR3(
       ...
       action::gotoJoints::Response &resp){
         ...
         for(int i=0; i<3; i++){
           ...
           delta_a = (pose.position.x - act_pose.transform.translation.x )/10;
           delta_b = (pose.position.y - act_pose.transform.translation.y )/10;
           delta_c = (pose.position.z - act_pose.transform.translation.z )/10;
           delta_d = (pose.orientation.x - act_pose.transform.rotation.x )/10;
           delta_e = (pose.orientation.y - act_pose.transform.rotation.y )/10;
           delta_f = (pose.orientation.z - act_pose.transform.rotation.z )/10;
           delta_g = (pose.orientation.w - act_pose.transform.rotation.w )/10;
           for (int i=0; i<10; i++){
             des_pose.position.x = act_pose.transform.translation.x + delta_a*(i+1);
             des_pose.position.y = act_pose.transform.translation.y + delta_b*(i+1);
             des_pose.position.z = act_pose.transform.translation.z + delta_c*(i+1);
             des_pose.orientation.x = act_pose.transform.rotation.x + delta_d*(i+1);
             des_pose.orientation.y = act_pose.transform.rotation.y + delta_e*(i+1);
             des_pose.orientation.z = act_pose.transform.rotation.z + delta_f*(i+1);
             des_pose.orientation.w = act_pose.transform.rotation.w + delta_g*(i+1);
             if(findIK(iksolution, des_pose))
             {
               ...
               closest_i = 0;
                 for(int i=0; i < iksolution.size(); i++){
                   dist = 0;
                   for (int j=0; j<6; j++){
                     dist = dist + pow(iksolution[i][j]-currentjoints[j],2);
                   }
                   if (dist < min_dist)
                   {
                     min_dist = dist;
                 ...
                 setRectilinearTrajectory(iksolution[closest_i], req.trajduration/10);
                 moveRobotTrajectory(req.trajduration/10);
             }
             else
             {
                return false;
